...
Casino.prototype.startup = function( conf ) {
	...
	var db = this.db = redis.createClient(conf.redis.port, conf.redis.host, {no_ready_check: true,password: conf.redis.auth.split(":")[1]});
	this.db.on('error', function(err){
		console.log('redis error (db):' + err.stack);
	});

	var pub = casino.pub = redis.createClient(conf.redis.port, conf.redis.host, {no_ready_check: true,password: conf.redis.auth.split(":")[1]});
	pub.on('error', function(err){
		console.log('redis error (pub): ' + err.stack);
	});

	var sub = casino.sub = redis.createClient(conf.redis.port, conf.redis.host, {no_ready_check: true,password: conf.redis.auth.split(":")[1]});
