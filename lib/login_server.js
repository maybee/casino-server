...
LoginServer.prototype.startup = function( conf ) {
	...
	this.pub = redis.createClient(conf.redis.port, conf.redis.host, {no_ready_check: true,password: conf.redis.auth.split(":")[1]});
	this.pub.on('error', function(err){
		console.log('redis error: ' + err);
	});

	this.sub = redis.createClient(conf.redis.port, conf.redis.host, {no_ready_check: true,password: conf.redis.auth.split(":")[1]});
